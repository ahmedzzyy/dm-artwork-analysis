...
from src.config import ARTISTS_CSV, ARTWORKS_CSV, MIN_CLASS_SAMPLES, TARGET_COL
...
def merge_datasets(artworks: pd.DataFrame, artists: pd.DataFrame) -> pd.DataFrame:
    ...
    # ConstituentID in artworks can look like "[123, 456]" — grab first ID"tqdm>=4.66"
...
def _parse_dimensions(series: pd.Series) -> pd.DataFrame:
    ...
    Extract width_cm and height_cm from the metric part.
    Returns a 2-col DataFrame.
    """

    def _extract(val):
        if pd.isna(val):
            return np.nan, np.nan
        # find all decimal/integer numbers after 'cm' hint
        cm_part = re.search(r"\(([^)]+cm[^)]*)\)", str(val))
        if not cm_part:
            return np.nan, np.nan
        nums = re.findall(r"[\d.]+", cm_part.group())
        nums = [float(n) for n in nums if float(n) < 10_000]
        if len(nums) >= 2:
            return nums[0], nums[1]  # h × w  (MoMA lists h first)
        return np.nan, np.nan
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Parse dates ─────────────────────────────────────────────────────
    df["creation_year"] = _parse_year(df["Date"])
    df["acquisition_year"] = _parse_year(df["DateAcquired"])
    df["artist_birth_year"] = _parse_year(
        df.get("ArtistBirthYear", pd.Series(dtype=str))
    )
    df["artist_death_year"] = _parse_year(
        df.get("ArtistDeathYear", pd.Series(dtype=str))
    )
    ...
    # ── Parse dimensions ────────────────────────────────────────────────
    dim_df = _parse_dimensions(df["Dimensions"])
    df = pd.concat([df, dim_df], axis=1)
    df["area_cm2"] = df["height_cm"] * df["width_cm"]

    # ── Gender → binary ─────────────────────────────────────────────────
    df["is_female"] = (
        df.get("ArtistGender", pd.Series(dtype=str))
        .astype(str)
        .str.lower()
        .str.contains("female")
        .astype("Int8")
...
def load_and_clean() -> pd.DataFrame:
    """
    End-to-end: load CSVs → merge → clean → return.
